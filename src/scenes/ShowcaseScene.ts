...
import type { AttackType, EnemyConfig, AttackCategory, PokemonForm } from '../types';
...
export class ShowcaseScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    // ── Título ─────────────────────────────────────────────────────
    this.add.text(width / 2, y, 'MODO TESTE - CHARMANDER LINE', {
    ...
    // ── Botão Voltar ────────────────────────────────────────────────
    y += 30;
    const backBtn = this.add.text(width / 2, y, '[ VOLTAR ]', {
      fontSize: '16px', color: '#ffcc00', fontFamily: 'monospace', fontStyle: 'bold',
    ...
    this.input.on('wheel', (_p: Phaser.Input.Pointer, _gos: Phaser.GameObjects.GameObject[], _dx: number, dy: number) => {
      ...
      cam.scrollY = Phaser.Math.Clamp(cam.scrollY + dy * 0.5, 0, this.contentHeight - cam.height);
    });
    ...
    this.input.on('pointermove', (p: Phaser.Input.Pointer) => {
      ...
      this.cameras.main.scrollY = Phaser.Math.Clamp(camStartY + dy, 0, this.contentHeight - this.cameras.main.height);
    });
  ...
  private addFormsSection(y: number): number {
    ...
    for (let i = 0; i < CHARMANDER_FORMS.length; i++) {
      ...
      this.add.text(fx, y + 70, form.name, {
        fontSize: '12px', color: '#ffffff', fontFamily: 'monospace', fontStyle: 'bold',
      }).setOrigin(0.5, 0);

      this.add.text(fx, y + 88, `Lv.${form.level} | ${form.maxAttackSlots}atk ${form.maxPassiveSlots}pas`, {
        fontSize: '9px', color: '#888888', fontFamily: 'monospace',
      }).setOrigin(0.5, 0);

      if (i < CHARMANDER_FORMS.length - 1) {
        this.add.text(fx + formSpacing / 2, y + 30, '→', {
  ...
  private addAttackRow(y: number, attackKey: AttackType, form: PokemonForm): number {
    ...
    const categoryColors: Record<AttackCategory, number> = {
      projectile: 0xff8800, orbital: 0x44aaff, cone: 0xcccccc,
      dash: 0x44ff44, area: 0xff44aa, aura: 0x888888,
    };
    ...
    // ── Nome do ataque ──────────────────────────────────────────────
    this.add.text(LABEL_X, y - 10, atk.name, {
      fontSize: '12px', color: '#ffffff', fontFamily: 'monospace', fontStyle: 'bold',
    }).setOrigin(0, 0.5);
    ...
    this.add.text(badgeX + 37, y + 11, category.toUpperCase(), {
      fontSize: '8px', color: Phaser.Display.Color.IntegerToColor(catColor).rgba,
      fontFamily: 'monospace', fontStyle: 'bold',
    }).setOrigin(0.5);

    // ── Descrição curta ─────────────────────────────────────────────
    this.add.text(LABEL_X + 85, y + 11, atk.description, {
    ...
    const elemColors: Record<string, string> = {
      fire: '#ff6600', normal: '#cccccc', dragon: '#7744ff', flying: '#88ccff',
    };

    this.add.text(STATS_X, y - 12, dmgText, {
      fontSize: '10px', color: '#ff6666', fontFamily: 'monospace', fontStyle: 'bold',
    }).setOrigin(0, 0.5);

    this.add.text(STATS_X + 65, y - 12, cdText, {
      fontSize: '10px', color: '#66aaff', fontFamily: 'monospace',
    }).setOrigin(0, 0.5);

    this.add.text(STATS_X, y + 8, elemText, {
      fontSize: '9px', color: elemColors[atk.element] ?? '#888888', fontFamily: 'monospace', fontStyle: 'bold',
    }).setOrigin(0, 0.5);

    this.add.text(STATS_X + 65, y + 8, `Lv max: ${atk.maxLevel}`, {
  ...
  private addAttackDemo(x: number, y: number, attackKey: AttackType): void {
    const spriteMap: Partial<Record<AttackType, { atkKey: string; animKey: string }>> = {
    ...
    if (!mapping) {
      this.add.text(x + 20, y, '[no sprite]', {
        fontSize: '9px', color: '#555555', fontFamily: 'monospace',
      }).setOrigin(0, 0.5);
      return;
    }
    ...
    if (!this.textures.exists(mapping.atkKey)) {
      this.add.text(x + 20, y, '[missing]', {
        fontSize: '9px', color: '#553333', fontFamily: 'monospace',
      }).setOrigin(0, 0.5);
      return;
    }
  ...
  private addEvolutionsSection(y: number): number {
    for (const evo of EVOLUTIONS) {
      ...
      // Base → Evolved
      this.add.text(LABEL_X, y - 10, `${baseAtk.name}  →  ${evoAtk.name}`, {
        fontSize: '12px', color: '#ffffff', fontFamily: 'monospace', fontStyle: 'bold',
      }).setOrigin(0, 0.5);

      // Requisitos
      this.add.text(LABEL_X, y + 10, `Lv${evo.requiredLevel} + ${evo.requiredItem} (${evo.requiredForm})`, {
        fontSize: '9px', color: '#888888', fontFamily: 'monospace',
      }).setOrigin(0, 0.5);
      ...
      // Descrição
      this.add.text(STATS_X, y, evo.description, {
  ...
  private addEnemiesSection(y: number): number {
    ...
    for (const key of enemyKeys) {
      ...
      // Nome
      this.add.text(LABEL_X + 70, y - 12, enemy.name.toUpperCase(), {
        fontSize: '12px', color: '#ffffff', fontFamily: 'monospace', fontStyle: 'bold',
      }).setOrigin(0, 0.5);

      // Stats
      const statsText = `HP:${enemy.hp}  SPD:${enemy.speed}  DMG:${enemy.damage}  XP:${enemy.xpValue}`;
      this.add.text(LABEL_X + 70, y + 8, statsText, {
        fontSize: '9px', color: '#aaaaaa', fontFamily: 'monospace',
      }).setOrigin(0, 0.5);
        ...
        if (this.textures.exists(projKey)) {
          ...
          const spawnProj = (): void => {
            ...
            this.tweens.add({
              ...
              x: DEMO_END_X,
              duration: 1500,
              onComplete: () => { if (projSprite.active) projSprite.destroy(); },
            });
          };

          spawnProj();
          const timer = this.time.addEvent({ delay: 2500, callback: spawnProj, loop: true });
          this.activeTimers.push(timer);

          // Stats do projétil
          const ra = enemy.rangedAttack;
          this.add.text(STATS_X, y, `Proj: DMG ${ra.damage} | SPD ${ra.speed} | CD ${(ra.cooldownMs / 1000).toFixed(1)}s`, {
